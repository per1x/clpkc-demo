...
// CL-PKC 充电桩客户端（生产化）。
//
// 通过 TCP 长连接与云平台完成：① HMAC 挑战-响应认证 → ② 经云平台向 KGC 申请
// 部分私钥 → ③ 带 Schnorr 签名的 ECDH 密钥协商 → ④ 会话密钥派生。
// 防重放仅依赖握手 nonce（无时间戳）。
// ============================================================================
...
int main(int argc, char** argv) {
    try {
        ...
        LOG_INFO("启动充电桩，静态公钥 P_i = " + static_key.public_hex);
        ...
        std::string cloud_static_public = field(auth, "publicKey");
        LOG_INFO("HMAC 认证通过。");
        ...
        std::string cloud_full_public = crypto.derive_full_public(field(ka, "publicKey"), field(ka, "derivedPublic"));
